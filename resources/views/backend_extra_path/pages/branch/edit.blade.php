        ...
        <div class="card card-default">
            ...
            <div class="card-body">
                ...
                <form class="needs-validation" method="POST"
                    action="{{ route('barnch-user-list.update',$editInfo->id) }}" novalidate>
                    @csrf
                    <div class="form-row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="branch_id" id="branch_id" class="form-control">
                                ...
                                <option value="{{$branch->id}}" {{ $branch->id == $editInfo->branch_id ? 'selected' : '' }}>{{$branch->name}}</option>
                                @endforeach
                            </select>
                            @error('branch_id')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control" id="validationCustom01"
                                placeholder="Branch User Name" value="{{ $editInfo->name ??'' }}">
                            @error('name')
                            <span class=" error text-red text-bold">{{ $message }}</span>
                            @enderror
                        </div>
