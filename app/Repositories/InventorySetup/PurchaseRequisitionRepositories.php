    ...
    public function getList($request)
    {
        ...
        if (empty($request->input('search.value'))) {
            ...
            if ($auth->branch_id !== null) {
                $prchaserequisitions = $prchaserequisitions->where('branch_id', $auth->branch_id);
            }
            ...
            $prchaserequisitions = $this->prchaserequisition::where('account_id', 'like', "%{$search}%");
            if ($auth->branch_id !== null) {
                $prchaserequisitions = $prchaserequisitions->where('branch_id', $auth->branch_id);
            }
            $prchaserequisitions = $prchaserequisitions->offset($start)
                ...
                // ->orderBy('status', 'desc')
                ->get();
            $totalFiltered = $this->prchaserequisition::where('account_id', 'like', "%{$search}%")->count();
        }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($category); $i++) {
                ...
                $purchasedetails->total_price = $total[$i];
                $purchasedetails->qty = $qty[$i];
                $purchasedetails->branch_id = $request->branch_id;
