...
class GrnRepositories
{
    ...
    public function getList($request)
    {
        ...
        if (empty($request->input('search.value'))) {
            ...
            $goodrcvnote = $goodrcvnote->where('purchase_type', 'Direct')
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                // ->orderBy('status', 'desc')
        ...
        if ($goodrcvnote) {
            foreach ($goodrcvnote as $key => $grn) {
                ...
                $nestedData['supplier_id'] = $grn->supplier->supplierCode . ' - ' . $grn->supplier->name;
                $nestedData['branch_id'] = $grn->branch->branchCode . ' - ' . $grn->branch->name;
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $goodrcvnote->supplier_id = $request->subblier_id;
            $goodrcvnote->purchase_voucher_id = $request->purchase_voucher;
            $goodrcvnote->branch_id = $request->branch_id;
            ...
            for ($i = 0; $i < count($category); $i++) {
                ...
                $existingCheck = StockSummary::where('product_id', $product[$i])->where('type', 'Branch')->where('branch_id', $request->branch_id)->first();
                // dd($existingCheck);
                if (!empty($existingCheck)) :
                    $newQty['quantity'] = $existingCheck->quantity + $remainingqty[$i];
                    StockSummary::where('product_id', $product[$i])->where('type', 'Branch')->where('branch_id', $request->branch_id)->update($newQty);
                else :
